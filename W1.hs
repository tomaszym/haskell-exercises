...
ilog2 x
    | restDiv2 == 1 = 1
    | restDiv2 > 1 = 1 + ilog2 (x `div` 2)
    | otherwise = 0
    where restDiv2 = x `div` 2
...
funnyMin x y = case funnyCompare x y of LT -> x
                                        GT -> y
                                        EQ -> x
...
wrap :: Integer -> String -> String
wrap (-1) s = s
wrap k s = wrap (k-1) (show k ++ "," ++ s ++ "," ++ show k)

pyramid :: Integer -> String
pyramid 0 = "0"
pyramid k = wrap (k-1) (show k)
...
smallestDivisor k = it k 2
it k l = if k `mod` l == 0 then l else it k (l+1)
...
nextPrime x = itp (x+1)
itp x = if isPrime x then x else itp (x+1)
