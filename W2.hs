...
mymax :: (a -> Int) -> a -> a -> a
mymax measure a b = case compare (measure a) (measure b) of LT -> b
                                                            GT -> a
                                                            EQ -> a
...
countSorted ss = length (filter (\a -> sort a == a) ss)
...
funny :: [String] -> String
funny strings = map (\a -> toUpper a) (intercalate " " (filter (\a -> length a > 5) strings))
...
powers :: Int -> Int -> [Int]
powers n max = takeWhile (\a -> a <= max) (map (\a -> n^a) [0..])
...
fromTo :: Int -> Int -> [Int]
fromTo n k = itf n k []
itf n k acc = if n > k then reverse acc else (itf (n+1) k (n:acc))
...
sums i = its (i+2) 2 [1]
its :: Int -> Int -> [Int] -> [Int]
its max i acc@(x:xs) = if max == i then reverse xs
                       else its max (i+1) ((x+i):acc)
...
mymaximum _ def [] = def
mymaximum _ _ (x:[]) = x
mymaximum cmp def (x:y:xs) = case cmp x y of
                                 GT -> mymaximum cmp def (x:xs)
                                 LT -> mymaximum cmp def (y:xs)
                                 EQ ->mymaximum cmp def (y:xs)
...
interpreter commands = iteri 0 0 commands
iteri a b [] = []
iteri a b (x:xs) = case x of
                         "incA" -> iteri (a+1) b xs
                         "incB" -> iteri a (b+1) xs
                         "decA" -> iteri (a-1) b xs
                         "decB" -> iteri a (b-1) xs
                         "printA" -> (show a) : iteri a b xs
                         "printB" -> (show b) : iteri a b xs
...
squares n = sqit 1 n
sqit _ 0 = []
sqit i n = if (head (show sqi) == last (show sqi)) then (toInteger sqi):sqit (i+1) (n-1)
           else sqit (i+1) n
           where sqi = i * i
